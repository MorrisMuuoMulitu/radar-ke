...
@torch.inference_mode()
def evaluate(pad_func, model, img_dir, save_dir, save_tag, collect_cases=False, case_dir=None):
    """Run inference over all volumes in img_dir.

    Same CSV output as the original script. When collect_cases is True,
    additionally saves one compressed .npz per volume into case_dir with:
      - image: preprocessed volume (float32, model's internal view)
      - mask: predicted organ mask, uint8 values 0..36 (0 = background)
      - columns: CSV column names for the score vector
      - scores: float32 score vector (NaN where a finding had no prediction)
    """
    ...
    for i, (image, test_items, meta_info) in enumerate(tqdm(dataloader, desc='Infer')):
        ...
        # get full mask
        # NOTE: 37-channel full-volume buffers live on CPU to fit <8GB GPUs.
        # Only the single-channel argmax result is moved back to GPU.
        ...
        for slice_g in range(0, num_win, sw_batch_size):
            ...
            # interpolate
            interpolated_seg_prob = F.interpolate(pred_window_seg_prob, size=window_patches.shape[2:], mode='trilinear').cpu()
        ...
        if collect_cases:
            ...
            score_vec = np.full(len(datafolder.test_items), np.nan, dtype=np.float32)
            for item, probs in organ_logits.items():
                score_vec[datafolder.test_items.index(item)] = np.concatenate(probs).mean(0)[1]
            case_path = os.path.join(case_dir, meta_info['file_name'] + '.npz')
            np.savez_compressed(
                case_path,
            ...
            print(f'case data saved to {case_path}')
...
def inference(initialize_returns, img_dir, save_dir, save_tag, collect_cases=False, case_dir=None):
    """
    Args:
    ...
    evaluate(pad_func, model, img_dir, save_dir, save_tag,
             collect_cases=collect_cases, case_dir=case_dir)
...
# ---------------------------------------------------------------------------
# Web-service API: process-wide model singleton + single-case / folder runners.
# ---------------------------------------------------------------------------
_MODEL_CACHE = None
...
def run_folder(img_dir, save_dir, save_tag, collect_cases=False, case_dir=None):
    """Run inference on every volume in img_dir. Returns the CSV path."""
    return inference(get_model(), img_dir, save_dir, save_tag,
                     collect_cases=collect_cases, case_dir=case_dir)


def run_case(nifti_path, out_dir):
    """Run inference on a single .nii/.nii.gz file.

    Returns dict with keys: file_name, csv_path, case_path (.npz with
    image/mask/columns/scores for the web viewer), scores ({column: prob}).
    """
    if not os.path.isfile(nifti_path):
        raise FileNotFoundError(nifti_path)
    case_in = os.path.join(out_dir, 'in')
    case_out = os.path.join(out_dir, 'out')
    os.makedirs(case_in, exist_ok=True)
    staged = os.path.join(case_in, os.path.basename(nifti_path))
    if os.path.abspath(staged) != os.path.abspath(nifti_path):
        import shutil
        shutil.copy2(nifti_path, staged)
    csv_path = run_folder(case_in, case_out, 'case',
                          collect_cases=True, case_dir=case_out)
    case_path = os.path.join(case_out, os.path.basename(nifti_path) + '.npz')
    data = np.load(case_path, allow_pickle=False)
    scores = {
        col: (None if np.isnan(v) else float(v))
        for col, v in zip([str(c) for c in data['columns']], data['scores'])
    }
    return {
