...
- Three-plane CT viewer with CT window presets, slice stepping, segmentation overlay controls, and finding-to-anatomy jumps.
- Finding table with search, anatomy filtering, thresholding, and CSV export.
- Review notebook with finding review states, shortlist, status, notes, JSON export, report text export, and full score export.
...
The report draft groups findings marked `Likely present` by anatomy in the structured confirmed-finding section. Shortlisted findings remain available for follow-up tracking and saved case history.
